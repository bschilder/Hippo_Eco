...
names(HP_orig)[names(HP_orig)=="Schizocortex"] <- "SCZ"
...
HP_orig$retroHP.SCZ <- HP_orig$retroHP + HP_orig$SCZ
...
# Create retoHP minus each subregion
  ...
  HP_orig$Brain__SCZ <- HP_orig$BrainVol - HP_orig$SCZ
...
eco_plots <- function(pgls_function.out, eco_var, run_eco.out){
  ...
  # only add linear fit if significant
  ifelse( p<=0.05, 
          eco_plot <- ggplot(data, aes(y=brain, x=eco_x, fill=Clade, color=Clade, shape=Clade)) + 
           ...
           geom_point(size=3) + 
           labs( y=paste(response_var,"res",sep="_"), x=paste(eco_var,sep="")) + 
           #title=paste("slope=", round(slope,4), "; p= ", round(p,4)),
           theme_classic() + 
           theme(plot.title = element_text(hjust=0.5,size=8), legend.position="none") +
           scale_shape_manual(values=shapes) + scale_color_manual(values=colors) + scale_fill_manual(values=colors), 
         
         eco_plot <- ggplot(data, aes(y=brain, x=eco_x, fill=Clade, color=Clade, shape=Clade)) + geom_point(size=3) +
           labs(y=paste(response_var,"res",sep="_"), x=paste(eco_var,sep="")) + 
           # title=paste("slope=", round(slope,4), "; p= ", round(p,4)), 
           theme_classic() + 
           theme(plot.title = element_text(hjust=0.5,size=8), legend.position="none") + 
...
all_eco_plots <- function(pgls_function.out){
  ...
  e1 <-eco_plots(p_f.o, eco_var = "DietBreadth", run_eco.out=eco)
  #e2 <-eco_plots(p_f.o, eco_var = "HomeRange_km2", run_eco.out=eco)
  #e3 <-eco_plots(p_f.o, eco_var = "HomeRange_Indiv_km2", run_eco.out=eco)
  e4 <-eco_plots(p_f.o, eco_var = "PopulationDensity_n_km2", run_eco.out=eco)
  ...
  e11 <-eco_plots(p_f.o, eco_var = "GroupSize_filled", run_eco.out=eco)
  e12 <-eco_plots(p_f.o, eco_var = "Home_Range", run_eco.out=eco)
  e13 <-eco_plots(p_f.o, eco_var = "Residual_HomeRange", run_eco.out=eco)
  
  title <- ggdraw() + draw_label(paste("Ecological Plots:",paste(pgls_function.out$response_var,"res",sep="_")),
                                 fontface='bold') 
  # gridExtra::grid.arrange(e1,e4,e11,e12,e13, ncol=3, 
  #                         top=paste("Ecological Plots:",paste(pgls_function.out$response_var,"res",sep="_")) ) #e6,e7,e10
    # mod_vars <- list(eco_vars=c("DietBreadth","PopulationDensity_n_km2","GroupSize_filled", "Home_Range", "Residual_HomeRange")) 
  subplot(e1,e4,e11,e12,e13, nrows = 2)
...
* Schizocortex: "Included are entorhinal, perirhinal, presubicular and parasubicular cortices and the underlying white matter. These cortices are characterized by the presence of one or several, almost cell-free layers of sublayers."
...
### Schizocortex vs. Medulla

NOTE: Using SCZ.med instead of SCZ.HP 1) flips the axes of the pPCA, and 2) gets rid of selective regime shift in human lineage. This is because humans are more divergent in SCZ.med (due to our massively enlarged neocotex, which EC is part of).
```{r Schizocortex vs. Medulla}
SCZ.Med_res <-pgls_function(HP, "SCZ", "Medulla")
run_eco(SCZ.Med_res);   all_eco_plots(SCZ.Med_res)
```

### Schizocortex vs. BrainVol

```{r Schizocortex vs. BrainVol}
SCZ.Brain_res <-pgls_function(HP, "SCZ", "Brain__SCZ")
run_eco(SCZ.Brain_res); all_eco_plots(SCZ.Brain_res)
```

### Schizocortex vs. retroHP * * *

Result: No relationship
```{r Schizocortex vs. retroHP}
SCZ.HP_res <-pgls_function(HP, "SCZ", "retroHP")
run_eco(SCZ.HP_res);   all_eco_plots(SCZ.HP_res)
#pgls_function(HP, "SCZ_res", "BrainVol")
```
...
### HP Fibers vs. retroHP + SCZ * * *

Result: No relationship
```{r HP Fibers vs. retroHP + SCZ}
fibers.HP_res <-pgls_function(HP, "Fibers", "retroHP.SCZ")
...
grid.arrange(HP.Med_res$xy_plot, HP.Brain_res$xy_plot, HPres.Enceph_res$xy_plot,
             ...
             ncol = 3, top = textGrob("Scaling Diagnostics", gp=gpar(fontsize=20)))
```

### SCZ & Fibers multiplots
```{r SCZ & Fibers multiplots}
grid.arrange(SCZ.HP_res$xy_plot, SCZ.Med_res$xy_plot, SCZ.Brain_res$xy_plot,
             fibers.HP_res$xy_plot, fibers.Med_res$xy_plot, fibers.brain_res$xy_plot, 
                 ncol=3, top=textGrob("SCZ & Fiber Regressions", gp=gpar(fontsize=20)))
```
...
grid.arrange(DG.HP_res$xy_plot, FD.HP_res$xy_plot, 
                 ...
                 ncol=4, top=textGrob("retroHP Subfield Regressions", gp=gpar(fontsize=20)))
```
...
# Absolute
subregion_proportions(HP$data, c("Sub", "CA1", "CA2", "CA3", "Hilus", "FD", "SCZ","Fibers")) 
...
neuro.summ <- rbind(
      ...
      data.frame(Residual="SCZ.HP_res", extract_neuro(SCZ.HP_res)),
...
pander(neuro.summ, caption="Neuroanatomical PGLS Regressions Summary", style="simple")
```
...
eco.summ <- rbind(
      ...
      data.frame(Residual="SCZ.HP_res", extract_eco(SCZ.HP_res)),
...
pander(eco.summ,  caption="Ecological PGLS Regressions Summary", style="simple")
...
## All residuals
all.vars <- c("retroHP_res", "SCZ_res", "Fibers_res", "DG_res", "FD_res", "Hilus_res", "CA2.3_res","CA3_res", "CA2_res", "CA1_res", "Sub_res")
## Simplified
vars <- c("SCZ_res", "Fibers_res", "FD_res", "Hilus_res","CA3_res", "CA2_res", "CA1_res", "Sub_res") 
...
# Plot PC1-eco
PCeco_plots <- function(eco_var, run_eco.out){
    pgls_eco_summary <- run_eco.out$pgls_eco_summary
...
ifelse(p<=0.05, 
       eco_plot <- ggplot(data, aes(y=brain, x=eco_x, fill=Clade, color=Clade, shape=Clade)) + 
         geom_smooth(inherit.aes=F, method="lm", color="black", data=data, aes(y=brain, x=eco_x)) + 
         geom_point(size=3) + 
         labs(y=paste(response_var,"res",sep="_"), x=paste(eco_var,sep="")) + 
         theme_classic() + 
         theme(plot.title = element_text(hjust=0.5,size=8), legend.position="none") +
...
PCall_eco_plots <- function(comp_data, response_var){
  ...
  e1 <-PCeco_plots(eco_var = "DietBreadth", run_eco.out=eco)
  e4 <-PCeco_plots(eco_var = "PopulationDensity_n_km2", run_eco.out=eco)
  e11 <-PCeco_plots(eco_var = "GroupSize_filled", run_eco.out=eco)
  e12 <-PCeco_plots(eco_var = "Home_Range", run_eco.out=eco)
  e13 <-PCeco_plots(eco_var = "Residual_HomeRange", run_eco.out=eco)
...
# Set up variables
cor.vars  <- c("BrainVol", "BrainVol_res", "retroHP", "retroHP_res", "SCZ_res", "Fibers_res", "DG_res", "FD_res", "Hilus_res", "CA2.3_res","CA3_res", "CA2_res", "CA1_res", "Sub_res")
...
#Convergence
  ...
  fit_ind <- estimate_shift_configuration(tree, Y, criterion="AICc",nCores=nCores)
  ## then detect which of these shifts are convergent:
  fit_conv <- estimate_convergent_regimes(fit_ind, criterion="AICc", nCores=nCores)
  fitconv_plot <- plot(fit_conv, edge.width=3); fitconv_plot
...
```{r Multivariate Analyses, fig.height=9, fig.width=9}
# qPCs 1-4
...
```{r Individual Subregions, fig.height=9, fig.width=9}
# retroHP_res only
HPres.pBIC <- l1ou(tree, dat=dplyr::select(plot.alldata,retroHP_res), model="pBIC")
# Individual subregions
SCZ.pBIC <- l1ou(tree, dat=dplyr::select(plot.alldata,SCZ_res), model="pBIC")
...
```{r pANCOVA, eval=F, include=F}
#library(devtools)   
#install_github("JeroenSmaers/evomap") #Needs to be installed only once
pANCOVA(phy, data, response_var, predictor_var){
library(ape)
library(geiger)
library(phytools)
library(nlme)
...
setwd("/Users/schilder/Desktop/Dissertation/HPeco/pANCOVA/") #Set your working directory to the folder to which you've downloaded the files. 
###############
#Load data and tree
      tree<-HP$phy
      data<-HP$data
...
#Run pgls
      #BM model
      model_pgls<-gls(retroHP~Brain__HP, data, corBrownian(phy=tree))
      model_pgls_summ <- summary(model_pgls); model_pgls_summ
      #Plot regression lines
      plot(log(data$retroHP) ~ log(data$Brain__HP)); abline(model_pgls)
###############
...
#Phylogenetic ancova
#groupings
      Human<-which(rownames(data)=="Homo_sapiens")
      GreatApes<-getTips(tree,findMRCA(tree,c("Homo_sapiens","Pan_troglodytes_verus","Gorilla_gorilla_gorilla")))
          GreatApes<-setdiff(GreatApes,Human)

###############
#Load the functions
      source("ancova_functions.r")

#2 groups
      #Set groups
            Groups<-rep("B",length(rownames(data)))
            Groups[c(Human,GreatApes)]<-"A"
            Groups<-as.factor(Groups)
      #Check groups
            tipCol<-rep("blue",length(tree$tip.label))
            tipCol[which(Groups=="A")]<-"red"
            plot(tree,tip.col=tipCol,cex=0.6)
            plot(Y~X,pch=21,bg=tipCol)
            abline(model_pgls)
      #Run ancova
            #Only 2 groups accepted for difference in slopes
            gls.ancova.defaultModels(Y,X,Sigma,Groups,test="Slopes")   
            #Maximum of 3 groups accepted for difference in intercepts
            gls.ancova.defaultModels(Y,X,Sigma,Groups,test="Intercepts")    

#3 groups
      #Set groups
            Groups<-rep("C",length(rownames(data)))
            Groups[Human]<-"A"
            Groups[GreatApes]<-"B"
            Groups<-as.factor(Groups)
      #Check groups
            tipCol<-rep("blue",length(tree$tip.label))
            tipCol[which(Groups=="A")]<-"red"
            tipCol[which(Groups=="B")]<-"green"
            plot(tree,tip.col=tipCol,cex=0.6)
            plot(Y~X,pch=21,bg=tipCol)
            abline(model_pgls)
      #Run ancova
            #Maximum of 3 groups accepted for diference in intercepts
            gls.ancova.defaultModels(Y,X,Sigma,Groups,test="Intercepts")     
output <- list(X=X, Y=Y, model_pgls_summ=model_pgls_summ)
...
# Draw HC Network
```{r Draw HC Network,warning=F,message=F}
species <- "Homo_sapiens"


n <- dplyr::select(HP$data, BrainVol_res,SCZ,FD,Hilus,CA3,CA2,CA1,Sub) 
...
melt.nodez <- melt(n,id.vars=c("Species","Clade"), measure.vars=
                     c("BrainVol_res","SCZ","FD","Hilus","CA3","CA2","CA1","Sub"),
...
links <- rbind(data.frame(from="BrainVol_res", to="SCZ"), 
               data.frame(from="SCZ", to="BrainVol_res"), 
               data.frame(from="SCZ", to="FD"), 
